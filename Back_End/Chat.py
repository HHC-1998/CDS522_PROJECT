...
class Chat:
    ...
    def aiTalk(self, question):
        ...
        # 第二次连接主要是为了回答用户
        # 第二个人工智能的身份
        firstAI = "查询结果应答助手”，核心职责：接收数据库返回的元组格式查询结果并分析其中的数据，结合客户原始问题，用自然语言总结回答；若结果为空你也要告知用户，要把元组中的具体信息都告诉用户。不要列一个数据串给我，而是解释给我听"
        response = client.chat.completions.create(
            model="deepseek-chat",
            messages=[
                {"role": "system", "content": firstAI},
